...
import numpy as np
from highlevel_verification_lib import BoxSort, ON_star, higher, scattered
from z3 import (
    Z3_OP_UNINTERPRETED,
    And,
    Const,
...
if __name__ == "__main__":
    ...
    b_prime, b, n, b0, a, m, z = Consts("b_prime b n b0 a m z", BoxSort)
    instructions = [
        Assign("b", "b0"),
        While(
            cond=(
            ...
            invariant=And(
                ...
                ForAll(
                    [m, n],
                    Implies(
                        And(ForAll([z], higher(z, m)), ForAll([z], higher(z, n))),
                        scattered(m, n),
                    ),
                ),
            ),
    ...
    precondition = And(
        ForAll(
            [m],
            And(ForAll([n], Or(m == n, Not(ON_star(n, m)))), ForAll([n], higher(n, m))),
        ),
        ForAll([m, n], scattered(m, n)),
    )

...
import gymnasium_robotics.utils.mujoco_py_utils as utils

import numpy as np
import mujoco_py
from environment.fetch_block_construction.xml import generate_xml
import tempfile

import pdb

class FetchBlockConstructionEnv(MujocoPyFetchEnv, EzPickle):
    def __init__(self, initial_qpos,
        ...
        with tempfile.NamedTemporaryFile(mode='wt', dir=F"{os.path.dirname(os.path.realpath(__file__))}/assets/fetch/", delete=False, suffix=".xml") as fp:
            fp.write(generate_xml(self.num_blocks))
            MODEL_XML_PATH = fp.name

        # MujocoFetchEnv.__init__(
        #     self, model_path=MODEL_XML_PATH, has_object=True, block_gripper=False, n_substeps=20,
        #     gripper_extra_height=0.2, target_in_the_air=True, target_offset=0.0,
        #     obj_range=0.15, target_range=0.15, distance_threshold=0.05,
        #     initial_qpos=initial_qpos, reward_type=reward_type, obs_type=obs_type, render_size=render_size, **kwargs)
        ...
        # EzPickle.__init__(self, initial_qpos, num_blocks, reward_type, obs_type, render_size, stack_only, **kwargs)
        EzPickle.__init__(self, initial_qpos, num_blocks, reward_type, obs_type, render_size, stack_only, **kwargs)
    ...
    # only for debug
    def get_custom_obs(self, obj_id):
        # positions
        ...
        achieved_goal = []
        object_i_pos = self.sim.data.get_site_xpos(self.object_names[obj_id])
        # rotations
        object_i_rot = rotations.mat2euler(self.sim.data.get_site_xmat(self.object_names[obj_id]))
        # velocities
        object_i_velp = self.sim.data.get_site_xvelp(self.object_names[obj_id]) * dt
        object_i_velr = self.sim.data.get_site_xvelr(self.object_names[obj_id]) * dt
        # gripper state
        ...
        obs = np.concatenate([
            ...
            grip_velp,
            gripper_vel,
            self.goal[obj_id*3:(obj_id+1)*3].copy()
        ])

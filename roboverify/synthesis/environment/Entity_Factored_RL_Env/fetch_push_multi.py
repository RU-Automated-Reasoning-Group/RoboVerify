        ...
        if self.num_objects > 0 or self.num_switches > 0:
            ...
            for i in range(self.num_objects):
                ...
                while True:
                    ...
                    for obj in prev_objects:
                        close_to_prev = close_to_prev or (np.linalg.norm(object_xpos - obj) < 0.1)
                    if dist_gripper < 0.1 or close_to_prev or object_xpos[0] > 1.46:
                        object_xpos = self.initial_gripper_xpos[:2] + self.np_random.uniform(
                            -self.obj_range, self.obj_range, size=2
                        )
                    else:
    ...
    def reset(self, **kwargs):
        ...
        while True:
            ...
            for i in range(self.num_objects):
                object_name = "object" + str(i)
                object_pos = self.sim.data.get_site_xpos(object_name)
                assert object_pos[0] <= 1.46
                if np.sum(np.linalg.norm(object_pos - obs['desired_goal_arr'], axis=-1) <= self.distance_threshold) > 0:
                    reset_success = False
                    break
            if reset_success:
                break
...
class FetchNPushObsWrapper(gym.ObservationWrapper):
    def __init__(self, env, env_success_rew=0):
        super().__init__(env)
        # observation space
        self.observation_space = spaces.Box(-np.inf, np.inf, \
                                            shape=(self.env.observation_space['observation'].shape[0]+self.env.observation_space['desired_goal'].shape[0],), \
                                            dtype='float32')

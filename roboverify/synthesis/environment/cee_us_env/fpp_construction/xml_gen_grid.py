...
from .xml_gen import BASIC_COLORS
...
def generate_xml_grid(num_blocks: int) -> str:
    if num_blocks <= 6:
    ...
    grid_marker_body = (
        '<body name="grid_marker{id}" mocap="true" pos="0 0 0">'
        '<geom size="0.012" rgba="0.55 0.30 0.08 0.55" type="sphere" '
    ...
    grid_markers = [grid_marker_body.format(id=i) for i in range(num_blocks)]
    block_bodies = [block_base.format(id=i) for i in range(num_blocks)]
    assets = [asset_base.format(id=i, color=colors[i]) for i in range(num_blocks)]

...
from synthesis.api.instructions import PickByName, MoveByName
...
from synthesis.mcmc.synthesis import make_roboverify_stack_env, preserved_global_rng, set_np_seed
...
class SegmentResetTests(unittest.TestCase):
    def test_reset_matches_action_replay_and_next_action_while_holding(self):
        with preserved_global_rng():
            ...
            try:
                program=Program(2,[PickByName("b0"),MoveByName("b0","b0","b0",target_offset=[0,0,.1])])
                states,record=collect_recording(program,env)
                trace=DemoTrace(states,tuple(record.snapshots),(tuple(record.actions),tuple(record.action_indices)),num_blocks=2)
                for t in sorted(set((0,len(states)//2,len(states)-1))):
                    ...
                    direct=capture(env)
                    action=np.array([0.,0.,.001,-.2])
                    next_direct=env.step(action)[0]
                    reset_segment(env,segment,mode="replay")
                    replay=capture(env)
                    next_replay=env.step(action)[0]
                    np.testing.assert_allclose(direct.gt_state,replay.gt_state,atol=1e-8,rtol=0)
                    np.testing.assert_allclose(next_direct,next_replay,atol=1e-8,rtol=0)
            finally: env.close()

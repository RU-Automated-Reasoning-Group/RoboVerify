...
def save_traces(path, traces):
    ...
    for i, trace in enumerate(traces):
        ...
        row={"seed":trace.seed,"task":trace.task,"num_blocks":trace.num_blocks,"snapshots":bool(trace.snapshots)}
        if trace.snapshots:
            ...
            for name in row["arrays"]:
                arrays[key+"_"+name]=np.stack([s.arrays[name] for s in trace.snapshots])
...
def load_traces(path):
    with np.load(path,allow_pickle=False) as data:
        ...
        if meta["version"] != 1: raise ValueError("Unsupported demo archive")
        traces=[]
        for i,row in enumerate(meta["traces"]):
            ...
            snapshots,actions=(),()
            if row["snapshots"]:
                snapshots=tuple(Snapshot(gt.copy(),{name:data[key+"_"+name][t].copy() for name in row["arrays"]},row["bindings"][t])
                                for t,gt in enumerate(data[key+"_gt"]))
                actions=(tuple(data[key+"_actions"].copy()),tuple(data[key+"_indices"].tolist()))
            traces.append(DemoTrace(states,snapshots,actions,row["seed"],row["task"],row["num_blocks"]))
        return traces

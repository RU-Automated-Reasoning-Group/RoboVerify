...
def scope(cfg):
    ...
    for edge in cfg.edges: universe.update(edge.binds)
    ...
    while changed:
        ...
        for vertex in sorted(reachable - {cfg.entry}):
            paths = [(values[e.source] | set(e.binds)) - set(e.kills)
                     for e in cfg.incoming(vertex) if e.source in reachable]
            new = set.intersection(*paths) if paths else set()
            if new != values[vertex]: values[vertex], changed = new, True

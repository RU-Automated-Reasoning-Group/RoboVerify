...
def evaluate_z3(expr, scene, bindings=None):
    ...
    def ev(node, bound=()):
        ...
        if z3.is_true(node): return True
        if z3.is_false(node): return False
        if z3.is_quantifier(node):
            values = (ev(node.body(), tuple(reversed(combo)) + bound)
                      for combo in itertools.product(scene.positions, repeat=node.num_vars()))
            return all(values) if node.is_forall() else any(values)
        if z3.is_and(node): return all(ev(a, bound) for a in node.children())
        if z3.is_or(node): return any(ev(a, bound) for a in node.children())
        if z3.is_not(node): return not ev(node.arg(0), bound)
        if z3.is_implies(node): return not ev(node.arg(0), bound) or ev(node.arg(1), bound)
        if z3.is_eq(node):
        ...
        predicates = {"ON_star": on.on_star_implementation, "ON_star_zero": on.on_star_implementation,
                      "Higher": on.higher_implementation, "Scattered": on.scattered_implementation}
        ...
        return bool(predicates[name](*(positions[term(a, bound)] for a in node.children())))
    return ev(expr)
...
def find_and_bind(instruction, env, traj):
    ...
    names = [str(v.decl().name()) if hasattr(v, "decl") else str(v) for v in instruction.guard_exists_vars]
    ...
    for values in itertools.product(scene.positions, repeat=len(names)):
        ...
        holds = evaluate(condition, scene, bindings) if isinstance(condition, Term) else evaluate_z3(condition, scene, bindings)
        if holds:

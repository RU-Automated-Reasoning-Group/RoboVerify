...
class SkillEnv(gym.Wrapper):
    def __init__(self, env, skill_graph, search_node, rew_fun, threshold=1.0, traj_len=100, hold_len=1, train=False, lagrangian=False, fail_search=False):
        super().__init__(env)
    ...
    def reset(self, **kwargs):
        ...
        while True:
            ...
            obs, info, success, traj_len = self.skill_graph.rollout(self.env, self.traj_len, self.search_node.node_id, drop_success=True, fail_search=self.fail_search, **kwargs)
    ...
    def step(self, action):
        ...
        # train mode
        if self.train:
            terminated = terminated or self.traj_id >= self.traj_len
            # penalty
        ...
        # eval mode
            ...
            # environment success
            if reward >= 0:
                terminated = self.traj_id >= self.traj_len
                new_reward = 1
            else:
                if new_reward >= self.done_thre:
                    ...
                    self.hold_step = 0
                terminated = self.hold_step >= self.hold_len or self.traj_id >= self.traj_len
...
class CustomMonitor(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        if self.reset_time % self.log_freq == 0:
            ...
            self.img_store.append(self.env.render(mode='rgb_array'))

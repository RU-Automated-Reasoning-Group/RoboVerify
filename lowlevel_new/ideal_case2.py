...
For collision, we do consider the length of the block.
"""
...
def visualize_scene(model, blocks, X, Y, Z, L=1.0):
    ...
    for b in blocks:
        cx = float(model.eval(X(b)).as_decimal(20))
        cy = float(model.eval(Y(b)).as_decimal(20))
        cz = float(model.eval(Z(b)).as_decimal(20))

        # Convert SMT decimal strings like "1.23?" to floats
        cx = float(str(model.eval(X(b)).as_decimal(20)))
        cy = float(str(model.eval(Y(b)).as_decimal(20)))
        cz = float(str(model.eval(Z(b)).as_decimal(20)))

        draw_cube(ax, cx, cy, cz, L)
...
def draw_vertical_swept_volume_from_block(
    ...
    def f(z3val):
        return float(str(model.eval(z3val).as_decimal(20)))
...
def ideal_on(b1, b2):
    return Or(box_equal(b1, b2), And(X(b1) == X(b2), Y(b1) == Y(b2), Z(b1) >= Z(b2)))
...
def scattered(t1, t2):
    return Or(Abs(X(t1) - X(t2)) > L, Abs(Y(t1) - Y(t2)) > L)

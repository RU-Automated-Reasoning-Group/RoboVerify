...
def loop_invariant(b):
    return And(ForAll([a], Exists([x], Or(ON_star(a, b0), top(a), And(top(x), ON_star(x, a))))), ON_star(b, b0), top(b))

def substituted_loop_invariant(x, b_prime, b):
    return And(
        ForAll([a], Exists([y], Or(Or(ON_star(a, b0), And(ON_star(a, b_prime), ON_star(b, b0))), substituted_top(a, b_prime, b), And(substituted_top(y, b_prime, b), Or(ON_star(y, a), And(ON_star(y, b_prime), ON_star(b, a))))))),

...
def loop_invariant(solver: Solver):
    ...
    solver.assert_and_track(Not(ON_star(tbl, b0)), "term2")
    solver.assert_and_track(
        ForAll([x], Or(Not(ON_star(b, x)), Not(ON_star(x, b0)))), "term3"
    )
    solver.assert_and_track(
        ForAll(
            [x, y],
...
def loop_invariant_combined(t):
    ...
    return And(
        ...
        ForAll(
            [x],
            Implies(And(Not(x == tbl), Not(ON_star(t, x))), ON_star(x, b0))
        ),
        Not(ON_star(tbl, b0)),
        ...
        ForAll(
            [x, y],
            Implies(And(ON_star_zero(y, x), Not(ON_star(x, b0))), ON_star(x, y))
        ),
        ForAll(
            [x, y],
            Or(Not(ON_star(x, y)), Not(ON_star(x, b0)), ON_star_zero(x, y))
...
# solver.assert_and_track(wp_for_put_on_box(b0, b, real_postcondition()), "not_post_cond_real")
# solver.assert_and_track(postcondition(), "postcondition")
# solver.assert_and_track(And(b0 != tbl, b != tbl), "b_b0_neq_tbl")
